...
const deleteDep = async (delete_id) => {
  await deleteDoc(doc(db, "departments", delete_id));
};
...
const updateDep = async () => {
  await updateDoc(doc(db, "departments", selectEmp.value), {
    name: n.value,
    address: a.value,
  });
  n.value = "";
...
<template>
  ...
  <div v-if="dep.length > 0" class="w-full py-6">
    <div class="h-screen w-full flex flex-col">
      ...
      >
        <div class="w-full max-w-md bg-gray-800 rounded-lg shadow-md p-6">
          <h2 class="text-2xl font-bold text-gray-200 mb-4">Add Department</h2>
      ...
      <div class="mt-5 px-5 w-full">
        <table class="w-full border-black border">
          ...
          <tbody v-for="d in dep" :key="d" class="border border-black">
            <td>{{ d.id }}</td>
            <td>{{ d.name }}</td>
            <td>{{ d.address }}</td>
            <td>
              <button
                @click="deleteDep(d.id)"
